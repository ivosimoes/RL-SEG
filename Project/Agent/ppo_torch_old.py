...
class ActorNetwork(nn.Module):
    ...
    def __init__(self, n_actions, input_units, alpha, hidden_units1=16, hidden_units2=256):
        super().__init__()
        ...
        self.actor = nn.Sequential(
            ...
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2),  # (B x H1 x 256 x 256)
            ...
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2), # (B x H1 x 32 x 32)
            
            nn.Flatten(1, -1),  # Note: Batch dimension doesn't exist. Should flatten H1 x 32 x 32.
            nn.Linear(in_features=hidden_units1*32*32, out_features=hidden_units1*8*8),
            nn.ReLU(),
            nn.Linear(in_features=hidden_units1*8*8, out_features=hidden_units2),
            nn.ReLU(),
...
class CriticNetwork(nn.Module):
    ...
    def __init__(self, input_units, alpha, hidden_units1=16, hidden_units2=256):
        super().__init__()
        ...
        self.critic = nn.Sequential(
            ...
            # Tip: Another Conv2d with stride 2 can be used instead of max pooling
