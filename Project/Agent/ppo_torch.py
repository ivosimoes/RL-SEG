...
class ActorNetwork(nn.Module):
    ...
    def __init__(self, n_actions, input_units, alpha, save_name, hidden_units1=16, hidden_units2=256):
        super().__init__()

        self.checkpoint_file = Path('../models/' + save_name + '_Actor.pth')
        self.actor = nn.Sequential(
            nn.Conv2d(input_units, hidden_units1, kernel_size=3, stride=2, padding=1),  # (B x H1 x 1024 x 1024)
            nn.ReLU(),
            nn.Conv2d(hidden_units1, hidden_units1, kernel_size=3, stride=2, padding=1),  # (B x H1 x 512 x 512)
            nn.ReLU(),
            nn.Conv2d(hidden_units1, hidden_units1, kernel_size=3, stride=2, padding=1),  # (B x H1 x 256 x 256)
            nn.ReLU(),
            nn.Conv2d(hidden_units1, hidden_units1, kernel_size=3, stride=2, padding=1),  # (B x H1 x 128 x 128)
            nn.ReLU(),
            nn.Conv2d(hidden_units1, hidden_units1, kernel_size=3, stride=2, padding=1),  # (B x H1 x 64 x 64)
            nn.ReLU(),
            nn.Conv2d(hidden_units1, hidden_units1, kernel_size=3, stride=2, padding=1),  # (B x H1 x 32 x 32)
            nn.ReLU(),
            nn.Conv2d(hidden_units1, hidden_units1, kernel_size=3, stride=2, padding=1),  # (B x H1 x 16 x 16)
            nn.ReLU(),
            nn.Conv2d(hidden_units1, hidden_units1, kernel_size=3, stride=2, padding=1),  # (B x H1 x 8 x 8)
            nn.ReLU(),
            nn.Conv2d(hidden_units1, 1, kernel_size=3, stride=2, padding=1),  # (B x H1 x 4 x 4)
            nn.ReLU(),
            
            nn.Flatten(1, -1),  # Note: Batch dimension doesn't exist. Should flatten H1 x 32 x 32.
            nn.Softmax(dim=-1)
        )
...
class CriticNetwork(nn.Module):
    ...
    def __init__(self, input_units, alpha, save_name, hidden_units1=32, hidden_units2=256):
        super().__init__()

        self.checkpoint_file = Path('../models/' + save_name + '_Critic.pth')
        self.critic = nn.Sequential(
            ...
            nn.Flatten(1, -1),  # Note: Batch dimension doesn't exist. Should flatten H1 x 32 x 32.
            nn.Linear(in_features=hidden_units1//2, out_features=1)
        )
...
class Agent:
    ...
    def __init__(self, n_actions, input_units, gamma=0.99, alpha=0.0003, policy_clip=0.2, 
                 gae_lambda=0.95, batch_size=64, N=2048, n_epochs=10, save_name='Torch_PPO'):
        self.gamma = gamma
        self.n_epochs = n_epochs
        self.policy_clip = policy_clip
        self.gae_lambda = gae_lambda
        
        self.actor = ActorNetwork(n_actions, input_units, alpha, save_name)
        self.critic = CriticNetwork(input_units, alpha, save_name)
